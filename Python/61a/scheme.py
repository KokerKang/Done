...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        check_procedure(evaluate)

        if isinstance(evaluate, MacroProcedure):
            return scheme_eval(procedure.apply_macro(rest, env), env)

        map_rest = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(evaluate, map_rest, env)
        # END PROBLEM 4
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError('incorrect number of arguments are passed')
