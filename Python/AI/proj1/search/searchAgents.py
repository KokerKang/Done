...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        self.cornerDit = {}
        for corner in self.corners: # (1,1), (1, top), (right, 1), (right, top)
            self.cornerDit[corner] = False  
            # {(1, 1): False, (1, top): False, (right, 1): False, (right, top): False}
            # to check the hit the goal corners
    ...
    def getStartState(self):
        ...
        print(self.cornerDit)
        """
        return (self.startingPosition, self.cornerDit)
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            nextCoordi = self.getNextState(state, action)
            actionCost = self.getActionCost(state, action, nextCoordi)
            children.append((nextCoordi, action, actionCost))
        self._expanded += 1 # DO NOT CHANGE
        return children
...
def cornersHeuristic(state, problem):
    ...
    for i in state[1]:
        ...
        if not(state[1][i]):
            result.append(i)
    if len(result) != 0:
        for coordi in result:
            temp.append(abs(state[0][0] - coordi[0]) + abs(state[0][1] - coordi[1]))
        return max(temp)
    else:
        return 0
